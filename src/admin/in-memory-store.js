...
export function createInMemoryStore() {
  ...
  return {
    ...
    async updateOrderFromWebhook({ provider, providerOrderId, status, eventId }) { if (eventId && events.has(eventId)) return { duplicate: true }; const providerKey = `${provider}:${providerOrderId}`; let order = orders.get(providerKey); if (!order) { order = { id: randomUUID(), provider, providerOrderId, status: "processing", buyerEmail: null, couponCode: null, subtotalCents: 0, discountCents: 0, totalCents: 0, lines: [], createdAt: now(), updatedAt: now(), couponRedeemed: false, reconciled: true }; orders.set(providerKey, order); } if (eventId) events.add(eventId); order.status = resolveOrderStatus(order.status, status); order.updatedAt = now(); const reservation = [...reservations.entries()].find(([, value]) => value.orderId === providerKey); if (order.status === "paid" && !order.couponRedeemed && order.couponCode) { const coupon = coupons.get(order.couponCode); if (!coupon || !reservation) throw new Error("Coupon reservation is missing."); coupon.redemptions += 1; order.couponRedeemed = true; reservations.delete(reservation[0]); } if (["failed", "refunded", "chargeback"].includes(order.status) && reservation) reservations.delete(reservation[0]); return copy(order); },
