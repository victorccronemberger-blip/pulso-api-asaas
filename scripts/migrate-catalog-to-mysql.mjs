  ...
  try {
    ...
    for (const [sortOrder, product] of catalog.entries()) {
      await connection.query(
        ...
        [product.slug, product.sourceTag, product.title, product.description ?? "", product.categoryId ?? "outros", product.kind ?? "course", product.accent ?? null, product.cohort ?? null, product.year ?? null, product.officialPriceCents, product.priceCents, product.featured ? 1 : 0, 1, sortOrder, `${assetOrigin}/media/pulso/v4/cards/${product.slug}.webp`, `${assetOrigin}/media/pulso/v4/cards/600/${product.slug}.webp`, `Card do curso ${product.title}`, JSON.stringify(product.keywords ?? [])],
      );
    }
