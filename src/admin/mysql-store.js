...
export function createMySqlStore(databaseUrl) {
  ...
  async function ensureSchema() {
    ...
    ready = (async () => {
      const statements = [
        ...
        `CREATE TABLE IF NOT EXISTS coupon_reservations (attempt_key CHAR(36) PRIMARY KEY, coupon_code VARCHAR(32) NOT NULL, appmax_order_id BIGINT UNSIGNED NULL UNIQUE, expires_at DATETIME(3) NOT NULL, created_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3), INDEX coupon_reservations_code_expiry (coupon_code, expires_at))`,
  ...
  return {
    ...
    async completeCheckoutAttempt(key, response) { await ensureSchema(); await pool.query("UPDATE checkout_attempts SET state='complete',response_json=CAST(? AS JSON),expires_at=DATE_ADD(NOW(3), INTERVAL 24 HOUR) WHERE idempotency_key=?", [JSON.stringify(response),key]); },
    ...
    async createOrder(order) { await ensureSchema(); const connection=await pool.getConnection(); try { await connection.beginTransaction(); const [[existing]]=await connection.query("SELECT id,status,coupon_redeemed FROM orders WHERE appmax_order_id=? FOR UPDATE",[order.appmaxOrderId]); const orderId=existing?.id??id(); const status=resolveOrderStatus(existing?.status,order.status); if(existing){await connection.query("UPDATE orders SET status=?,buyer_email=?,subtotal_cents=?,discount_cents=?,total_cents=?,coupon_code=? WHERE id=?",[status,order.buyerEmail,order.subtotalCents,order.discountCents,order.totalCents,order.couponCode,orderId]);await connection.query("DELETE FROM order_items WHERE order_id=?",[orderId]);}else{await connection.query("INSERT INTO orders (id,appmax_order_id,status,buyer_email,subtotal_cents,discount_cents,total_cents,coupon_code) VALUES (?,?,?,?,?,?,?,?)",[orderId,order.appmaxOrderId,status,order.buyerEmail,order.subtotalCents,order.discountCents,order.totalCents,order.couponCode]);} for(const line of order.lines)await connection.query("INSERT INTO order_items (order_id,course_slug,title,base_price_cents,discount_cents,final_price_cents) VALUES (?,?,?,?,?,?)",[orderId,line.product.slug,line.product.title,line.basePriceCents,line.discountCents,line.finalPriceCents]); if(order.checkoutAttemptKey){const [bound]=await connection.query("UPDATE coupon_reservations SET appmax_order_id=? WHERE attempt_key=?",[order.appmaxOrderId,order.checkoutAttemptKey]);if(!bound.affectedRows)throw new Error("Coupon reservation is missing.");} if(status==='paid'&&order.couponCode&&!existing?.coupon_redeemed){await connection.query("INSERT INTO coupon_redemptions (coupon_code,order_id) VALUES (?,?)",[order.couponCode,orderId]);await connection.query("UPDATE orders SET coupon_redeemed=1 WHERE id=?",[orderId]);await connection.query("DELETE FROM coupon_reservations WHERE attempt_key=?",[order.checkoutAttemptKey]);} await connection.commit();return{id:orderId,...order,status};}catch(error){await connection.rollback();throw error;}finally{connection.release();} },
    async updateOrderFromWebhook({ appmaxOrderId,status,eventId }) { await ensureSchema(); const connection=await pool.getConnection(); try { await connection.beginTransaction(); if(eventId){const [seen]=await connection.query("SELECT event_id FROM webhook_events WHERE event_id=?",[eventId]); if(seen.length){await connection.rollback();return {duplicate:true};}} let [[order]]=await connection.query("SELECT * FROM orders WHERE appmax_order_id=? FOR UPDATE",[appmaxOrderId]); if(!order){const reconciledId=id();await connection.query("INSERT INTO orders (id,appmax_order_id,status,buyer_email,subtotal_cents,discount_cents,total_cents,coupon_code) VALUES (?,?,?,NULL,0,0,0,NULL)",[reconciledId,appmaxOrderId,status]);order={id:reconciledId,status,coupon_code:null,coupon_redeemed:0};} const nextStatus=resolveOrderStatus(order.status,status); if(eventId)await connection.query("INSERT INTO webhook_events (event_id) VALUES (?)",[eventId]); await connection.query("UPDATE orders SET status=? WHERE id=?",[nextStatus,order.id]); if(nextStatus==='paid'&&!order.coupon_redeemed&&order.coupon_code){const [[reservation]]=await connection.query("SELECT attempt_key FROM coupon_reservations WHERE appmax_order_id=? FOR UPDATE",[appmaxOrderId]); if(!reservation)throw new Error("Paid order has no coupon reservation."); await connection.query("INSERT INTO coupon_redemptions (coupon_code,order_id) VALUES (?,?)",[order.coupon_code,order.id]);await connection.query("UPDATE orders SET coupon_redeemed=1 WHERE id=?",[order.id]);await connection.query("DELETE FROM coupon_reservations WHERE attempt_key=?",[reservation.attempt_key]);} if(['failed','refunded','chargeback'].includes(nextStatus))await connection.query("DELETE FROM coupon_reservations WHERE appmax_order_id=?",[appmaxOrderId]); await connection.commit();return{id:order.id,status:nextStatus};}catch(e){await connection.rollback();throw e;}finally{connection.release();} },
