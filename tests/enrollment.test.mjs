...
async function serve(context, { behavior = confirmed, maxRetries = 3, retryDelayMs = 1, overrides = {} } = {}) {
...
async function createPaidCandidateOrder(store, { providerOrderId, cpf = "19100000000" } = {}) {
...
test("confirms a course enrollment automatically when the payment webhook turns an order paid", async (context) => {
  const { base, store, service } = await serve(context);
  await createPaidCandidateOrder(store, { providerOrderId: "pay_enroll1" });
...
test("does not enroll twice for duplicate or non-transitioning webhooks", async (context) => {
  const { base, store, service } = await serve(context);
  await createPaidCandidateOrder(store, { providerOrderId: "pay_enroll2" });
  ...
  // Duplicate event id and a fresh event that no longer transitions the status.
  await postWebhook(base, "pay_enroll2", "evt_enroll2&1");
...
test("marks an enrollment failed after exhausting retries", async (context) => {
  ...
  await createPaidCandidateOrder(store, { providerOrderId: "pay_enroll4" });
...
test("admin can list enrollments and requeue a failed job back to confirmed", async (context) => {
  ...
  await createPaidCandidateOrder(store, { providerOrderId: "pay_enroll5" });
