...
export function createMySqlStore(databaseUrl) {
  ...
  return {
    ...
    async listCatalogProducts({ activeOnly = true } = {}) {
      await ensureSchema();
      const [rows] = await pool.query(`SELECT slug,source_tag AS sourceTag,title,description,category_id AS categoryId,kind,accent,cohort,course_year AS year,official_price_cents AS officialPriceCents,price_cents AS priceCents,featured,active,sort_order AS sortOrder,keywords_json AS keywordsJson FROM products ${activeOnly ? "WHERE active=1" : ""} ORDER BY sort_order ASC,title ASC`);
