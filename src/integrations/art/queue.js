...
// Fila serializada de matriculas. Um job por vez porque o login na plataforma ART
// revoga o token RS256 anterior — dois logins paralelos da mesma conta quebram o
// polling. A persistencia (tabela enrollments) e a fonte de verdade: o processo
// pode reiniciar que os jobs 'processing' viram 'queued' de novo no boot.
export function createEnrollmentQueue({ store, enrollmentService, environment, log = console.log }) {
  ...
  async function enqueueOrder(order) {
    ...
    if (!order.buyerEmail || !order.buyerCpf) {
      log(`[enrollment] order ${order.id} paid but missing buyer email/cpf — cannot auto-enroll`);
